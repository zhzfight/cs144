...
uint64_t unwrap(WrappingInt32 n, WrappingInt32 isn, uint64_t checkpoint) {
    ...
    quotient = (checkpoint - absolute_seqno) >> 32;
    remainder = ((checkpoint - absolute_seqno) << 32) >> 32;
    if (remainder < size_period / 2)
        return absolute_seqno + quotient * size_period;
    else
        return absolute_seqno + (quotient + 1) * size_period;
